prompt HELP_DESK REFERENCE module
prompt (c) 2020 Greenwhite solutions (www.greenwhite.uz)
----------------------------------------------------------------------------------------------------
create table hd_quiz_set_groups(
  company_id                      number(20)          not null,
  quiz_set_group_id               number(20)          not null,
  name                            varchar2(300 char)  not null,
  state                           varchar2(1)         not null,
  constraint hd_quiz_set_groups_pk primary key (company_id, quiz_set_group_id) using index tablespace GWS_INDEX,
  constraint hd_quiz_set_groups_c1 check (state in ('A', 'P'))
) tablespace GWS_DATA;
...
---------------------------------------------------------------------------------------------------- 
create table hd_option_quiz_sets(
   company_id                     number(20)    not null,
   option_id                      number(20)    not null,
   child_quiz_set_id              number(20)    not null,
   constraint hd_option_quiz_sets_pk  primary key (company_id, option_id, child_quiz_set_id)  using index tablespace GWS_INDEX, 
   constraint hd_option_quiz_sets_f1 foreign key (company_id, child_quiz_set_id) references hd_quiz_sets(company_id, quiz_set_id),
   constraint hd_option_quiz_sets_f2 foreign key (company_id, option_id) references hd_quiz_options(company_id, option_id) on delete cascade
) tablespace  GWS_DATA;

----------------------------------------------------------------------------------------------------
create table hd_option_quizs(
   company_id                     number(20)    not null,
   option_id                      number(20)    not null,
   child_quiz_id                 number(20)     not null,
   constraint hd_option_quizs_pk primary key (company_id, option_id, child_quiz_id) using index tablespace GWS_INDEX, 
   constraint hd_option_quizs_f1 foreign key (company_id, option_id) references hd_quiz_options(company_id, option_id) on delete cascade,
   constraint hd_option_quizs_f2 foreign key (company_id, child_quiz_id) references hd_quizs(company_id, quiz_id)
) tablespace  GWS_DATA;
 
----------------------------------------------------------------------------------------------------
create table hd_quiz_set_group_binds(
  company_id                      number(20)    not null,
  quiz_set_group_id               number(20)    not null,
  quiz_set_id                     number(20)    not null,
  order_no                        number(20),
  constraint hd_quiz_set_group_binds_pk primary key (company_id, quiz_set_group_id, quiz_set_id) using index tablespace GWS_INDEX,
  constraint hd_quiz_set_group_binds_f1 foreign key (company_id, quiz_set_group_id) references hd_quiz_set_groups(company_id, quiz_set_group_id),
  constraint hd_quiz_set_group_binds_f2 foreign key (company_id, quiz_set_id) references hd_quiz_sets(company_id, quiz_set_id)
) tablespace GWS_DATA;
